    ...
    for m in months:
        try:
            for k,v in city.items():
                # 找到json格式数据的url
                url = "http://tianqi.2345.com/t/wea_history/js/"+str(y)+str(m)+'/'+str(v)+"_"+str(y)+str(m)+".js"
                print(url)
                ...
                for item in aqi:
                    cit.append(k)
                data_spider = pd.DataFrame([date,maxtemp,mintemp,tianqi,winddir, wind, aqi,aqiinfo,aqilevel,cit]).T
